...
# from .bot_lib.utils import (
#     bot_edit_callback_message,
#     bot_safe_send_message,
#     bot_safe_send_photo,
# )
...
#
# @dp.message_handler(
#     lambda message: features.creator_ftr.find_triggers(message), creator=True
# )
# async def creator_filter_check(msg: types.Message) -> None:
#     await msg.answer(
#         features.creator_ftr.text, parse_mode=types.ParseMode.MARKDOWN
#     )
#
#
# @dp.message_handler(
#     Text(equals=features.cancel_ftr.triggers, ignore_case=True), state='*'
# )
...
#
# @dp.callback_query_handler(
#     game_cb.filter(action=features.start_ftr.callback_action), registered=True
# )
# @dp.message_handler(
#     Text(equals=features.start_ftr.triggers, ignore_case=True), registered=True
# )
...
#
# @dp.message_handler(
#     Text(equals=features.help_ftr.triggers, ignore_case=True), registered=True
# )
...
#     await bot_safe_send_message(
#         dp, from_user_id, text, reply_markup=features.start_ftr.kb
#     )
#
#
# # -------------------------------------------- PROFILE HANDLERS -------------------------------------------------------
# @dp.message_handler(
#     Text(equals=features.set_user_info.triggers, ignore_case=True),
#     registered=True,
# )
# async def set_name(msg: types.Message) -> None:
#     await msg.answer(
#         features.set_user_info.text, reply_markup=features.cancel_ftr.kb
#     )
...
#     await fill_form(
#         msg=msg, feature=features.set_user_name, form=UserForm, state=state
#     )
#
#
...
#     await fill_form(
#         msg=msg, feature=features.set_user_about, form=UserForm, state=state
#     )
#
#
# async def fill_form(
...
#             tg_user=tg_user, user_form_data=user_form_data
#         )
#     await state.finish()
#     await msg.answer(
#         features.set_user_info.text2, reply_markup=features.set_user_info.kb
#     )
#
#
...
# # -------------------------------------------- GAME HANDLERS ----------------------------------------------------------
# @dp.message_handler(
#     Text(equals=features.rating_ftr.triggers, ignore_case=True),
#     registered=True,
# )
...
#     text = features.rating_ftr.text.format(
#         user_taps=user.taps, total_taps=total_taps
...
#         )
#         await msg.answer(text, reply_markup=features.rating_ftr.kb)
#         await bot_safe_send_photo(
#             dp,
#             msg.from_user.id,
#             best_user.photo,
#             reply_markup=features.rating_ftr.kb,
#         )
#
#
# @dp.message_handler(
#     Text(equals=features.press_button_ftr.triggers, ignore_case=True),
#     registered=True,
# )
...
#     registered=True,
# )
# async def count_button_tap(
#     callback: types.CallbackQuery, callback_data: dict[Any, Any]
# ) -> None:
#     current_taps = int(callback_data['taps'])
#     new_taps = current_taps + 1
#     await db.incr_user_taps(
#         tg_user=TgUser(
#             tg_id=callback.from_user.id, username=callback.from_user.username
#         )
#     )
#     text, keyboard = await update_button_tap(taps=new_taps)
#     await bot_edit_callback_message(
#         dp, callback, text, reply_markup=Feature.create_tg_inline_kb(keyboard)
#     )
#
#
# async def update_button_tap(
#     *, taps: int
...
#                 callback_data=game_cb.new(
#                     action=features.press_button_ftr.callback_action, taps=taps
#                 ),
#             )
#         ],
#         [
...
#                 callback_data=game_cb.new(
#                     action=features.start_ftr.callback_action, taps=taps
#                 ),
#             )
#         ],
#     ]
...
#                 Errors.please_register, reply_markup=features.empty.kb
#             )
#         if not msg.from_user.username:
#             return await msg.answer(
#                 Errors.register_failed, reply_markup=features.empty.kb
#             )
#     # user registration
...
#         await bot_safe_send_message(
#             dp, int(settings.creator_id), features.ping_ftr.text2
...
#     bot_commands = [
#         types.BotCommand(ftr.slashed_command, ftr.slashed_command_descr)
#         for ftr in cmds
#     ]
